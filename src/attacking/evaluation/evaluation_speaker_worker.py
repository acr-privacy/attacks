...
from attacking.common.config import SPEAKER_LIBRISPEECH_TEST_LABELENCODER, SPEAKER_YOUTUBE_TEST_LABELENCODER
from attacking.attacks import sonnleitner_attack, acrcloud_attack, zapr_attack
from tensorflow.keras.callbacks import TensorBoard
from sklearn.preprocessing import label_binarize
from sklearn.metrics import auc, classification_report, f1_score, precision_score, recall_score, roc_curve
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mt', '--method', type=str, required=True, choices=['ACRCLOUD', 'SONNLEITNER', 'ZAPR_ALG1', 'ZAPR_ALG2'])
    parser.add_argument('-d', '--dataset', type=str, required=True, choices=['LIBRISPEECH', 'YOUTUBE'])
    ...
    model_params = {'n_classes': 40 if args.dataset == 'LIBRISPEECH' else 50}
    ...
    labelencoder = load(globals()[f"SPEAKER_{args.dataset}_TEST_LABELENCODER"])
    ...
    model.fit(x_train, y_train, validation_data=(x_test, y_test), callbacks=callbacks, verbose=0, **fit_params)
    ...
    with open(os.path.join(args.split_folder, 'classification_report.log'), 'w') as f:
        f.write(classification_report(y_test, y_pred, target_names=[str(x) for x in labelencoder.classes_], digits=4))

    # dump the macro-average f1, precision, recall
    macro_scores = dict()
    macro_scores["f1"] = f1_score(y_test, y_pred, average='macro')
    macro_scores["precision"] = precision_score(y_test, y_pred, average='macro')
    macro_scores["recall"] = recall_score(y_test, y_pred, average='macro')

    split_results = {metric_name: score for metric_name, score in zip(model.metrics_names, test_scores)}
    ...
    n_classes = 40 if args.dataset == 'LIBRISPEECH' else 50
    ...
    dump(y_test_binarized, os.path.join(args.split_folder, 'y_test_binarized.joblib.gz'))
