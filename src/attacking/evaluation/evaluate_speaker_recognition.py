...
from attacking.common.config import DATAPATHS, SPEAKER_LIBRISPEECH_TEST_LABELENCODER, SPEAKER_YOUTUBE_TEST_LABELENCODER
...
def make_mean_roc_std_plot(fprs, tprs, roc_aucs, ax, cv_runs=10):
    ...
    all_fpr = np.unique(np.concatenate([macro_fprs[i] for i in range(cv_runs)]))  # all needed fpr thresholds (x-values)
...
def which_indices(x_coords, y1_coords, y2_coords=None, threshold=PLOT_THRESHOLD):
    ...
    if y2_coords is not None and len(y2_coords) != len(y1_coords):
        raise Exception("Invalid shape. y2_coords needs to have same dimensions as x and y1.")
    used_indices = [0]
    cur_x = x_coords[0]
    cur_y1 = y1_coords[0]
...
def get_limited_lists(x_coords, y1_coords, y2_coords=None, threshold=PLOT_THRESHOLD, return_indices=False):
    ...
    """
    used_indices = which_indices(x_coords, y1_coords, y2_coords=y2_coords, threshold=threshold)
...
def plot_extrema_mean_rocs(fprs, tprs, roc_aucs, n_classes, cv_runs=10):
    ...
    speaker_mean_aucs = [(spkr, np.mean(speaker_aucs[spkr])) for spkr in range(n_classes)]
    ...
    all_best_fpr = np.unique(np.concatenate([best_fprs[run] for run in range(cv_runs)]))
    ...
    all_worst_fpr = np.unique(np.concatenate([worst_fprs[run] for run in range(cv_runs)]))
    ...
    lim_all_best_fpr, lim_mean_best_tpr = get_limited_lists(all_best_fpr, mean_best_tpr, threshold=PLOT_THRESHOLD)
    ...
    lim_all_worst_fpr, lim_mean_worst_tpr = get_limited_lists(all_worst_fpr, mean_worst_tpr, threshold=PLOT_THRESHOLD)
...
def call_worker(params):
    ...
    """
    split_folder, method, dataset = params
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'evaluation_speaker_worker.py')
    print(f"Starting worker for {split_folder}.")
    subprocess.call(['python', script_path, '--method', method, '--dataset', dataset, '--split_folder', split_folder])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='LIBRISPEECH',
                        choices=['LIBRISPEECH', 'YOUTUBE'],
                        help='Select test dataset.')
    ...
    n_classes = 40 if used_dataset == 'LIBRISPEECH' else 50
    if not args.plot_only:
        labelencoder = load(globals()[f"SPEAKER_{used_dataset}_TEST_LABELENCODER"])

        data = extract_file_contents(DATAPATHS[f"{main_method}_{used_dataset}_SPEAKER_RECOGNITION_TEST"], mode='speaker_librispeech')
        ...
        if args.method == 'SONNLEITNER':
            ...
            feature_extract_params = {'skip_bytes': [2, 3, 4, 5], 'max_bits': 2496}
        ...
        mock_data = np.zeros(len(data))  # Used for kfold.split insted of data DF (see kfold documentation)

        # dump data and features
        store_path = os.environ['EXPERIMENT_ROOT']
        ...
        with Pool(processes=args.n_processes) as p:
            p.map(call_worker, [(split_folder, args.method, args.dataset) for split_folder in cv_out_roots])
    else:
        cv_out_roots = [os.path.join(args.outfolder, f'split_{i}') for i in range(cv_runs)]
    ...
    for split_folder in cv_out_roots:
        ...
        split_results = load(os.path.join(split_folder, 'split_results.joblib.gz'))
        for metric_name, score in split_results.items():
            results[metric_name].append(score)
        macro_files = load(os.path.join(split_folder, 'macro_scores.joblib.gz'))
    ...
    plot_final(fprs, tprs, roc_aucs,
               n_classes=n_classes,
               outpath=os.path.join(args.outfolder, f"evaluation_plot_spkr_mean_std_{args.method}_no_bounds.png"),
               cv_runs=cv_runs)
    plot_final(fprs, tprs, roc_aucs,
               n_classes=n_classes,
               outpath=os.path.join(args.outfolder, f"evaluation_plot_spkr_mean_std_{args.method}_no_bounds.pdf"),
               cv_runs=cv_runs)
    plot_final(fprs, tprs, roc_aucs,
               n_classes=n_classes,
               outpath=os.path.join(args.outfolder, f"evaluation_plot_spkr_mean_std_{args.method}_no_bounds.tex"),
               cv_runs=cv_runs)
