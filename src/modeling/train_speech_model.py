...
from features.acrcloud import preprocess_acrcloud_speechvsmusic_dataset
from features.sonnleitner import preprocess_sonnleitner_speechvsmusic_dataset
from features.zapr1 import preprocess_zapr1_speechvsmusic_dataset
from features.zapr2 import preprocess_zapr2_speechvsmusic_dataset


def run_speech_experiment(
    model: keras.Model,
    datadir: Path,
    outdir: Path,
    method: str,
    ...
    extract_functions = {
        "acrcloud": preprocess_acrcloud_speechvsmusic_dataset,
        "sonnleitner": preprocess_sonnleitner_speechvsmusic_dataset,
        "zapr_alg1": preprocess_zapr1_speechvsmusic_dataset,
        "zapr_alg2": partial(
            preprocess_zapr2_speechvsmusic_dataset, n_bits=n_bits
        ),
    }

    meta, x, y = extract_functions[method](datadir / method, seq_len=seq_len)

    test_idx = meta.split == "testing"
    x_test = x[test_idx]
    y_test = y[test_idx]

    train_idx = meta.split == "training"
    x_train = x[train_idx]
    y_train = y[train_idx]

    val_idx = meta.split == "validation"
    x_val = x[val_idx]
    y_val = y[val_idx]
